...
extern float vision_yaw_kal_perdict;
void DataWave(UART_HandleTypeDef* huart)
{	
		//Channel：选择通道（1-10）
		//传入单精度浮点数
	  DataScope_Get_Channel_Data((float) gimbal.pid.yaw_angle_ref , 1 );
	  DataScope_Get_Channel_Data((float) gimbal.pid.yaw_angle_fdb , 2 );
		DataScope_Get_Channel_Data((float) gimbal.pid.yaw_spd_ref, 3 );
	  DataScope_Get_Channel_Data((float) gimbal.pid.yaw_spd_fdb, 4 );
